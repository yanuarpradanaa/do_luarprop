...
class stock_picking(models.Model):
    ...
    finishing = fields.Char(related="pack_operation_ids.finishing", string="Finishing")
    sides = fields.Char(related="pack_operation_ids.sides", string="Sides")
    product_category = fields.Char(related="pack_operation_ids.product_category", string="Product Category")
    def _prepare_pack_ops(self, quants, forced_qties):
        ...
        for move in self.move_lines.filtered(lambda move: move.state not in ('done', 'cancel')):
            ...
            if values and values[0]:
                values[0].update({'product_description': move.product_description,'finishing': move.finishing,'sides':move.sides,'product_category':move.product_category})
...
class stock_pack_operation(models.Model):
    ...
    finishing = fields.Char("Finishing")
    sides = fields.Char("Sides")
